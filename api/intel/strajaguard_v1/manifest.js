...
export default async function handler(req, res) {
  ...
  const key = `${BUNDLE_MODEL_KEY}/${version}/manifest.json`;

  try {
    const result = await fetchBundleObject(version, 'manifest.json');
    ...
    return res.status(status).json({ error: 'Failed to fetch manifest' });
  }
}
